...
Security: Ensured API keys are handled via .env and not hardcoded in the repository.
Data Integrity: Verified that the file chunking strategy correctly splits text files into manageable context windows.
UI/UX: Tested the responsiveness and flow of the "Inject Data" -> "Query" -> "Verify" cycle.
